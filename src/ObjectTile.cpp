#
#include "include/ObjectTile.hpp"

ObjectTile::ObjectTile(float obX, float obY, sf::Vector2f gridsize_f, sf::Texture& texture, const sf::IntRect& texturerect, short object_type)
    : Tile(TileTypes::OBJECT, obX, obY, gridsize_f, texture, texturerect, false)
{
    ...
    this->rect.setTextureRect(sf::IntRect(sf::Vector2i(0, 0), sf::Vector2i(9, 16)));
    this->rect.setPosition(sf::Vector2f(obX, obY));
...
const std::string ObjectTile::asString() const
{
    ...
    out << std::hex << this->type << " " << this->rect.getTextureRect().position.y << " " << this->rect.getTextureRect().position.y << " " << this->object_type << " " << std::setprecision(20) << this->rect.getPosition().x << " " << std::setprecision(20) << this->rect.getPosition().y;
    return out.str();
}
...
{
    out << std::hex << this->type << " " << this->rect.getTextureRect().position.y << " " << this->rect.getTextureRect().position.y << " " << this->object_type << " " << std::hexfloat << this->rect.getPosition().x << " " << std::hexfloat << this->rect.getPosition().y << " ";
}
