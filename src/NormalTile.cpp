...
#
#include "include/NormalTile.hpp"

NormalTile::NormalTile(short type, float x, float y, sf::Vector2f gridsize_f, sf::Texture& texture, const sf::IntRect& texturerect, bool collision_enabled)
    : Tile(type, x, y, gridsize_f, texture, texturerect, collision_enabled)
{
}

NormalTile::NormalTile(short int type,short guid, sf::Vector2f gridsize_f, sf::Texture& texture, bool collision_enabled) : Tile(type,guid,texture,collision_enabled)
{
...
const std::string NormalTile::asString() const
{
    ...
    out << std::hex << this->type << " " << this->rect.getTextureRect().position.x << " " << this->rect.getTextureRect().position.y << " " << this->collison_enabled;
