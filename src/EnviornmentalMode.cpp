...
#
#include "include/EnviornmentalMode.hpp"
...
void EnviornmentalMode::inittext()
{
    ...
    this->cursortext = new sf::Text(*this->editorstatedata->font, "", GUI::calcCharSize(vm, 100));
    this->cursortext->setFillColor(sf::Color::White);
    // this->cursortext.setCharacterSize(GUI::calcCharSize(vm, 100));
    this->cursortext->setOutlineThickness(1.f);
    this->cursortext->setPosition(sf::Vector2f(GUI::pixelpercentX(73.4, vm), GUI::pixelpercentX(3, vm)));
    ...
    this->text_container.setPosition(sf::Vector2f(GUI::pixelpercentX(71.4, vm), GUI::pixelpercentY(0, vm)));
...
void EnviornmentalMode::initGUI()
{
    ...
    this->guisprite = new sf::Sprite(this->guitexture);
    this->guisprite->setTexture(this->guitexture);
    this->guisprite->setScale(sf::Vector2f(4.f, 4.f));
    this->sidebar.setSize(sf::Vector2f(sf::Vector2f(80.f, static_cast<float>(this->statedata->gfxsettings->resolution.size.y))));
    ...
    this->select_Rect.setSize(this->statedata->gridSize);
...
EnviornmentalMode::EnviornmentalMode(StateData* statedata, TileMap* tilemap, EditorStateData* editorstatedata)
...
{
    if ((sf::Mouse::isButtonPressed(sf::Mouse::Button::Left) && this->getkeytime()) || (sf::Keyboard::isKeyPressed(sf::Keyboard::Key(this->editorstatedata->keybinds->at("PLACE"))))) {
    ...
    // Remove a tile
    else if ((sf::Mouse::isButtonPressed(sf::Mouse::Button::Right) && this->getkeytime()) || (sf::Keyboard::isKeyPressed(sf::Keyboard::Key(this->editorstatedata->keybinds->at("REMOVE"))) && this->getkeytime())) {
        // if the user's mouse is not on the sidebar
        if (!this->sidebar.getGlobalBounds().contains(sf::Vector2f(*this->editorstatedata->mousePosWindow))) {
