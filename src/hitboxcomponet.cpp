...
HitBoxComponet::HitBoxComponet(sf::Sprite& sprite, float offset_x, float offset_y, float width, float height)
    : sprite(sprite)
    , offsetX(offset_x)
    , offsetY(offset_y)
{
    ...
    this->nextpos.position.x = 0.f;

    this->nextpos.position.y = 0.f;
    this->nextpos.size.x = width;
    this->nextpos.size.y = height;
}
...
void HitBoxComponet::update()
{
    ...
    this->hitbox.setPosition(sf::Vector2f(this->sprite.getPosition().x + this->offsetX, this->sprite.getPosition().y + this->offsetY));
}
...
bool HitBoxComponet::intersects(const sf::FloatRect& rect)
{
    ...
    std::optional<sf::FloatRect> result = this->hitbox.getGlobalBounds().findIntersection(rect);
...
void HitBoxComponet::setposition(const sf::Vector2f positon)
{
    ...
    this->sprite.setPosition(sf::Vector2f(positon.x - this->offsetX, positon.y - this->offsetY));
}
...
void HitBoxComponet::setposition(const float x, const float y)
{
    ...
    this->sprite.setPosition(sf::Vector2f(x - this->offsetX, y - this->offsetY));
}
...
const sf::FloatRect& HitBoxComponet::getNextPosition(const sf::Vector2f& velocity)
{
    ...
    this->nextpos.position.x = this->hitbox.getPosition().x + velocity.x;
    this->nextpos.position.y = this->hitbox.getPosition().y + velocity.y;
