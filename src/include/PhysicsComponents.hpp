...
#ifndef PhysicsComponents_hpp
#define PhysicsComponents_hpp
...
public:
    ...
    /// @param Deceleration set a rate of deceleration
     PhysicsComponents(sf::Sprite& sprite, float maxVelocity, float Acceleration, float Deceleration);
    ...
    ///Fetch the acceleration of the entity
     [[nodiscard]] const float& getAcceleration() const;

     [[nodiscard]] const float& getDecceleration() const;
    ///@brief getStatus
    ///@discussion  Fetch wether or not the entity is moving in a particular direction
    ///@param state the direction to check
     [[nodiscard]] bool getStatus(const short unsigned& state) const;
    
    
     bool getMoveTime();
    
};
#endif /* PhysicsComponents_hpp */
